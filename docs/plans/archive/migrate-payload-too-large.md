...
- [x] Phase 3: モバイル側 timeout 緩和 + リカバリ動線整備
  - [x] `mobile/src/api/migrate.ts` で timeout 120s を渡す
  - [x] `runLoginMigration` の catch で 413 / ECONNABORTED 分岐 + 日本語メッセージ
  - [x] 失敗 Alert を「破棄してログイン続行」「もう一度試す」「キャンセル」の 3 択に
  - [x] `__tests__/utils/migration.test.ts` に上記ケースを追加
