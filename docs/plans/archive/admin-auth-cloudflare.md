...
### Phase 6 — ドキュメントと CLAUDE.md 反映 ✅ 完了 (2026-04-25)
...
**実装結果:**
- 更新: `README.md` の環境変数表に `CF_ACCESS_TEAM_DOMAIN` / `CF_ACCESS_AUD` /
  `ADMIN_AUTH_DEV_BYPASS` / `ADMIN_AUTH_DEV_EMAIL` を追記、`JWT_SECRET` の説明を
  「アプリ向け Magic Link / Google OAuth 用」と明記。表の直下に
  「管理画面はアプリ認証と独立し本番では Cloudflare Access の Google SSO 前段
  ゲート＋ Origin の `Cf-Access-Jwt-Assertion` 検証で守る」旨と本 plan への
  リンクを追加
- 更新: `CLAUDE.md`「アーキテクチャ上の注意」を改訂
  - アプリ向け認証 = `requireAuth` (`req.userId` / `req.userEmail`)、
    管理画面 = `requireCloudflareAccess` (`req.adminEmail`) で完全別系統
  - モバイルから `/api/admin` を叩かない／`requireAuth` と
    `requireCloudflareAccess` を重ね掛けしない、をそれぞれ明記
  - 続けて新セクション「ローカルで管理画面を触るとき」で
    `server/.env` に `ADMIN_AUTH_DEV_BYPASS=1` / `ADMIN_AUTH_DEV_EMAIL=dev-admin@local`
    を書く手順を記載（`dev-admin/README.md` は存在しないため CLAUDE.md に集約）
- 更新: `TODO.md` から本タスク行と全 Phase チェックを削除し、
  `DONE.md` に Phase 1〜6 の完了エントリを追加
