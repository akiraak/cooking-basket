...
- **Step 0-6**: **「passkey を入れる意味はあるか？」の最終判定**（本プランを Phase 1 以降に進める前のゲート）
  - 比較表 (a)(b)(c)(d) を Step 0-1〜0-5 の事実関係を踏まえて更新し、passkey 自体の ROI を再評価
  - 評価軸: (i) UX 改善幅、(ii) 個人開発の運用負荷（ライブラリ追従 / AASA & Team ID 変更追従 / Cloudflare 配信維持）、(iii) 代替案 (a)(b)(c) で十分に UX 改善が達成されるか
  - 「(a)+(c) で十分」「(a) だけで体感差が出る」など、passkey 不要と判断できれば **本プランは Phase 0 で終了**（Phase 1 以降は実行しない）

**判断ポイント**: 上記 Step が全部埋まる。配信ルート未確定 / 端末 OS 不足 / Team ID 取得不可
のいずれかで詰むなら **No-Go**（または当該事項を解消するまで保留）。
さらに **Step 0-6 で「passkey を入れる意味なし」と結論が出た場合も No-Go** として
本プランを Phase 0 で打ち切り、判断根拠だけ残してアーカイブする。

#### Phase 0 確認結果（2026-04-27）

リポジトリから確定できる事項のみ先行記録。**未確定の Step 0-1 / 0-2 / 0-5 / 0-6 はユーザー
側の確認・判断待ち**。

- **Step 0-1: Apple Team ID** — **未取得**。`mobile/app.json` および `mobile/eas.json`
  のいずれにも Team ID は記載なし（`eas.json:24` の `submit.production.ios.ascAppId: "6760154283"`
  のみ）。`eas credentials` または Apple Developer Portal から取得が必要
- **Step 0-2: 手元実機の OS バージョン** — **未確認**。実機側でのみ確定可能。Phase 3 着手前に
  iPhone iOS 16+ / Android 9+ + Google Play Services 23.40+ を満たすか実機で要確認
- **Step 0-3: `.well-known` 配信ルート** — **確定**。Express の `app.use(express.static(webDir))`
  (`server/src/app.ts:74`、`webDir = path.join(__dirname, '../../web')`) が `web/` 配下を
  そのまま配信する。AASA / assetlinks は `web/.well-known/apple-app-site-association` と
  `web/.well-known/assetlinks.json` を新規作成すれば配信される（`web/.well-known/` は現状未存在）。
  本番経路は Cloudflare → cloudflared Tunnel → cooking-basket コンテナ（`docs/plans/archive/admin-auth-cloudflare.md`
  Phase 0 で確定済み）。Cloudflare Access の `basket-admin-allow` ポリシーは `/admin` と
  `/api/admin` にのみ attach されており `/.well-known/*` には影響しない（同 Phase 1 の確認結果）。
  Cloudflare の Auto Minify / Browser Integrity Check / Bot Fight Mode などが JSON を
  改変しないか・リダイレクトが入らないかは Phase 2 で `curl -IL` で確認する
- **Step 0-4: モバイル Google OAuth 現状** — **確定（実装なし）**。`mobile/src/api/auth.ts`
  は `requestLogin` / `verifyCode` / `getMe` の 3 関数のみで Google OAuth を呼んでいない。
  `mobile/src/stores/auth-store.ts` も OTP フローしか扱わない。`mobile/` 配下に `google`
  を含む実装参照は無い（`mobile/src/`, `mobile/app.json`, `mobile/eas.json`,
  `mobile/package.json` を grep）。サーバの `/api/auth/google` は実装されているが、
  **モバイルから到達する経路は存在しない**ため、本プラン背景の「Google OAuth はフォールバック」
  という記述は誤り。**現実のフォールバックは Magic Link / OTP のみ**
- **Step 0-5: JWT 90 日 + Refresh Token 先行着手** — **判断待ち**。比較表 (a) は 0.5〜1 人日
  かつ UX 改善度「中」で ROI が高い。本検証と独立して TODO 化することを推奨するが、
  着手判断はユーザーに委ねる
- **Step 0-6: passkey 着手是非の最終判定** — **判断待ち**。Step 0-1 / 0-2 の取得と Step 0-5
  の判断が揃った段階で、比較表 (a)(b)(c)(d) を更新したうえで Go / No-Go を確定する

##### Phase 0 最終判定: **No-Go**（2026-04-27）

Step 0-3 / 0-4 のリポジトリ事実関係と、別途行った技術調査（ライブラリ健全性・代替案 ROI）を踏まえ、
**本検証プランを Phase 0 で打ち切る**ことを決定した。Step 0-1（Apple Team ID）/ 0-2（実機 OS）の
取得は不要となったため未実施のまま終了。Step 0-5（JWT 90 日 + Refresh Token 着手）と (b)/(c) の
代替案は **本プランの結論を受けて独立 TODO として切り出す**（本プランの判断材料には使うが、
本プランの中では実装しない）。

###### 判断根拠

1. **「passkey を入れる意味」が現時点で薄い**
   - 既存フローは Magic Link（OTP）のみで、ユーザーには既に **「パスワードを入れない」UX**
     を提供できている。passkey の差分は「メールを開く手間が消える」のみ
   - JWT を 30 → 90 日にするだけで再ログイン頻度は月 1 回未満になる見込みで、
     体感 UX の改善幅としてはこちらが先（比較表 (a)、0.5〜1 人日）
   - お料理バスケットは iOS 審査中・Android クローズドテスト中で実ユーザー数が僅少。
     初期段階で 3〜4 週間を passkey 検証 + 本実装に投じる ROI が無い
2. **個人開発の運用負荷が高い**
   - RN 側の主力ライブラリ `react-native-passkey`（f-23）は **bus factor 1**（個人メンテ）。
     Expo SDK の年次バージョンアップに毎回追従コストが発生する見込み
   - Cloudflare Bot Fight Mode が Apple AASA fetcher を 403 にする事例が 2024〜2025 で複数報告。
     `.well-known/*` 配下のセキュリティ緩和ルールを CF 側で維持する負担が増える
   - iCloud Keychain / Google Password Manager の同期問題（機種変更で消えた・家族 Apple ID 共有で
     見えない・Android↔iOS は QR スキャン必須）に起因するサポート負荷が個人開発者ブログで
     繰り返し報告されている。フォールバック（OTP）併設は不可避で運用負担は減らない
3. **代替手段で十分カバーできる見込み**
   - (a) **JWT 90 日 + Refresh Token**（0.5〜1 人日 / UX 改善「中」/ サーバのみ）— 最優先。
     これだけで再ログイン頻度問題はほぼ解決する
   - (b) **Apple Sign In**（5〜7 日 / iOS で 1 タップ / iOS 審査要件としても妥当）—
     iOS の体感を伸ばすならこちら
   - (c) **Google OAuth モバイルネイティブ化**（5〜7 日 / Android で 1 タップ / **サーバ側
     `/api/auth/google` は実装済みで未使用**）— サーバ実装が既にあるためモバイル側の追加工数のみ
   - これらを順に入れた後、MAU が伸びて「ログイン面倒」のフィードバックが複数来てから
     passkey を再評価するのが ROI 的に妥当

###### 残タスク（独立 TODO 化推奨）

- **(a) JWT 90 日 + Refresh Token 化**（最優先 / サーバのみ）
- **(b) Apple Sign In** / **(c) Google OAuth ネイティブ化**（ストア審査要件・OS ごとの体感改善
  観点で順次検討）
- **passkey 再評価** — MAU が伸び、ユーザーから「ログイン面倒」の声が複数集まった段階で
  本プランを再開判断。最短でも 6〜12 か月後の見込み

参考: 検証中に行った技術調査（ライブラリ健全性・代替案比較）はこの判断の根拠として残置。
詳細を再現する場合は `docs/plans/archive/passkeys-auth-verification.md` の本セクションを参照。か
