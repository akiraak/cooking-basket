...
### Phase 2: SSE 変更通知
- [ ] 5. `GET /api/files/watch` を `text/event-stream` で実装
- [ ] 6. `fs.watch` で `TODO.md` / `DONE.md` を監視し、`stat` で mtime 確認後に `event: change` を送る
- [ ] 7. 2 秒ポーリングのフォールバックを追加（WSL2 保険）
- [ ] 8. 接続切断時の watcher 解除と keep-alive ping（30 秒）を実装
- [ ] 9. (動作確認) `curl -N http://localhost:3010/api/files/watch` を開いた状態で別端末から `echo >> TODO.md` して通知が届くか確認
