...
- [ ] 未ログイン/ログイン/有料の機能分けユーザー視点も考慮しながら明確にする資料の作成 [plan](docs/plans/feature-tier-matrix.md)
- [ ] 未ログインでも AI 呼び出しができる現状にセキュリティ・コスト面の問題がないか調査（不正利用・トークン濫用・レート制限・課金影響を整理し、必要なら認証/レート制限の対策案を出す） [plan](docs/plans/ai-unauth-abuse-review.md)
