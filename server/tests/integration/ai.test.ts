...
describe('POST /api/ai/suggest', () => {
  ...
  it('ignores legacy mode field (returns recipes + ingredients regardless)', async () => {
    // 旧クライアントが mode を送ってきても無視して通常の挙動になることを確認
    const res = await request(app)
      .post('/api/ai/suggest')
      .set('X-Device-Id', 'dev-legacy-mode')
      .send({ dishName: 'カレー', mode: 'ingredients' });
